...
int ds4_create()
{
	...
	strcpy((char*)ev.u.create2.name, "Sony Corp. DualShock 4 [CUH-ZCT1x]");
...
int ds4_send_report(const char *report, size_t nbytes)
{
	...
	memset(&ev, 0, sizeof(ev));
	ev.type = UHID_INPUT2;
	ev.u.input.size = nbytes;
    memcpy(ev.u.input.data, report, nbytes);
	return uhid_write(&ev);
}
...
int ds4_recieve_req()
{
    fputs("DS4 Checking for requests\n", stderr);
    poll(&ds4_pfd, 1, 0);
